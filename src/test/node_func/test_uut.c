...
void write_compute_node_properties(pbsnode &reporter, FILE *nin);
void add_to_property_list(std::string &property_list, const char *token);
int login_encode_jobs(pbsnode *pnode, tlist_head *phead);
int cray_enabled;
...
void initialize_allnodes(all_nodes *an, pbsnode *n1, pbsnode *n2)
  {
  an->lock();
  an->clear();
...
void add_prop(pbsnode &pnode, const char *prop_name)
  {
